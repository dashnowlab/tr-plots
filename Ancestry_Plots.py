...
def create_horizontal_bar_plot(filtered_df, gene, disease, original_df):
    ...
    for pore_type in trace_order:
        ...
        # Build population counts subtitle for this pore type
        pop_df = df_grouped[['Cleaned population description', 'total_count']].copy()
        ...
        pop_counts_str = "<br>".join([
            f"<span style='font-size:12px'>Total Individuals per Population ({pore_type}): {line}</span>" if i == 0 else
            f"<span style='font-size:12px'>{line}</span>"
            for i, line in enumerate(pop_lines)
        ])
        trace_map[pore_type + '_pop_counts_str'] = pop_counts_str
    ...
    for key in trace_order:
        if key in trace_map:
            ...
            # Store population counts string for each pore type
            pop_counts_per_pore.append(trace_map[key + '_pop_counts_str'])

    if 'All Types' in trace_labels:
        fig.data[trace_labels.index('All Types')].visible = True

    # Default annotation for population counts (All Types)
    default_pop_counts = pop_counts_per_pore[trace_labels.index('All Types')]
