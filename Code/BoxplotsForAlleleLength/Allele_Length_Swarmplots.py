...
def create_boxplot(_, gene, disease, original_df):  # `_` placeholder for unused argument
    filtered_df = original_df[(original_df['Gene'] == gene) & (original_df['Disease'] == disease)]
    ...
    fig = px.strip(
        ...
        y='SuperPop',
        title=title_html,
        stripmode='overlay',  # this is default; keeps all points visible
    )
...
for gene in df_agg['Gene'].unique():
    ...
    for disease in diseases:
        ...
        fig = create_boxplot(None, gene, disease, df)
        fig.show()
        if not TEST_MODE:
            if fig:
