...
# Load Sample Population Information (no headers assumed to handle unnamed last column)
sample_info = pd.read_csv(TSV_PATH, sep="\t", skiprows=1)

# Print shape to debug if needed
# print("TSV shape:", sample_info.shape)

# Use sample_id (col 0), column 2 (population), and column 3 (population_description)
sample_info = sample_info.iloc[:, [0, 2, 3]]
sample_info.columns = ['sample_id', 'population', 'population_description']

# Create Mapping Dictionary (remove duplicates first)
population_map = (
    sample_info
    .drop_duplicates(subset="sample_id")
    .set_index("sample_id")[["population", "population_description"]]
...
for record in vcf_in.fetch():
    ...
    for sid, sample in record.samples.items():
        ...
        pop_info = population_map.get(base_id, {"population": "Unknown", "population_description": "Unknown"})
        ...
        records.append({
            ...
            "population_description": pop_info["population_description"],
            "motif": motif,
...
# # output_file = "/Users/annelisethorn/Documents/Anschutz/Code/Matching Files/CSVs/68_loci_100_samples_Matching_JSON_TSV.csv"
# df.to_csv(output_file, index=False)
# print(f"Saved CSV File")

# # Save to Excel
# # excel_output_file = "/Users/annelisethorn/Documents/Anschutz/Code/Matching Files/Excels/68_loci_100_samples_Matching_JSON_TSV.xlsx"
