...
OUTPUT_FILE = '/Users/annelisethorn/Documents/github/tr-plots/data/other_data/updated_v6_all_path_ref_motif_allele_spreadsheet.xlsx'
...
def _longest_pure_repeat_count(sequence: Optional[str], motif: Optional[str]) -> float:
    ...
    within `sequence`. If either is missing, returns NaN.
    """
    ...
    pat = re.compile(f"(?:{re.escape(motif_clean)})+")
    best = 0
...
def _allele_sequence(allele_tok: str, ref: str, alt_list: List[str]) -> Optional[str]:
    """
    Return the raw allele sequence for the given GT token:
      - '0' => REF sequence
      - integer n => ALT[n-1] sequence
      - invalid/missing => None
    """
    if allele_tok in (None, '.', ''):
        return None
    if allele_tok == '0':
        return ref
    try:
...
def _motif_metrics(sequence: Optional[str], motif: Optional[str]) -> dict:
    ...
    # longest contiguous run: (mot)+
    run_pat = re.compile(r'(?:' + re.escape(mot) + r')+', flags=re.IGNORECASE)
    best_units = 0
    best_span = (None, None)
    ...
    # total occurrences (allow overlapping) using lookahead
    occ_pat = re.compile(r'(?=(' + re.escape(mot) + r'))', flags=re.IGNORECASE)
    ...
    if best_units > 0 and total_occ > best_units:
        ...
        # motif appears but never as >=1 full contiguous repeat unit? treat as interruption/no pure run
        has_interruptions = True
