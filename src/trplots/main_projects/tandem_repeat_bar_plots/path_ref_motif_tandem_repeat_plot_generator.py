...
def main(args=None):
    ...
    # --- Output directories (computed after parsing for consistency) ---
    out_base = Path(args.output_dir) if args.output_dir else (Path(OUTPUT_BASE) / "plots" / "path_ref_motif_tandem_repeats_plots")
    if TEST_MODE:
        OUTPUT_HTML_DIR = os.path.join(out_base, "test_outputs")
        OUTPUT_PNG_DIR  = OUTPUT_HTML_DIR
        os.makedirs(OUTPUT_HTML_DIR, exist_ok=True)
    else:
        OUTPUT_HTML_DIR = os.path.join(out_base, "html")
        OUTPUT_PNG_DIR  = os.path.join(out_base, "png")
        os.makedirs(OUTPUT_HTML_DIR, exist_ok=True)
